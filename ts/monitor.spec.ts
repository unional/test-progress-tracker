...
import { append, init, type MonitorSubscription, monitor, type TestResults } from '.'
...
test('callback invoked with last save entry initially', async () => {
	...
	try {
		...
		const actual = await new Promise((a) => {
			sub = monitor({ rootDir, awaitWriteFinish: { pollInterval: 10, stabilityThreshold: 50 } }, (_, testResults) =>
				a(testResults)
			)
		})
